...
┌─────────────────┐      ┌─────────────────┐      ┌─────────────────┐
│ IBTrACS Tracks  │      │   NWM Basins    │      │ GAGES II / III  │
└────────┬────────┘      └────────┬────────┘      └────────┬────────┘
│                        │                        │
└───────────┬────────────┘                        │
▼                                     │
...
1. **Data Ingestion & Standardized Reprojection**: Automates fetching remote NetCDF, GeoPackage, Shapefile, and CSV data sources defined in `configuration.json`[cite: 8]. Geometries are clean-projected to North America Equidistant Conic (`ESRI:102010`)[cite: 9, 10].
2. **Spatial Distance Intersection**: Buffers National Water Model (NWM) assimilation basin boundaries to extract tropical cyclone tracks within $400\text{ km}$ of each river basin, generating site-specific storm temporal windows (`start` to `end`)[cite: 10, 14].
3. **Partitioned Streamflow Ingestion**: Tracks progress via a localized SQLite checkpoint manifest (`streamflow_manifest.sqlite3`)[cite: 8, 13]. Batched asynchronous requests retrieve continuous USGS unit-value streamflow observations during storm windows, saving records to a Hive-partitioned Parquet dataset (`prefix=XX/year=YYYY/`)[cite: 11, 12].
4. **Lazy Peak Discharge Aggregation**: Uses Polars lazy frames to execute exact multi-column joins (`usgs_site_code`, `prefix`, `year`) against the Hive partition tree, extracting maximum volumetric flow rate (`peak_streamflow_cfs`) per storm event[cite: 12].

---

## Data Sources

The framework automatically ingests and attributes the following datasets[cite: 8]:

| Dataset | Format | Source | Description |
| :--- | :--- | :--- | :--- |
| **IBTrACS** | NetCDF (`.nc`) | NOAA NCEI | Global tropical cyclone best track archive (1980–Present)[cite: 8]. |
| **GAGES-II** | Shapefile (`.zip`) | USGS | Geospatial attributes and reference basin boundaries[cite: 8]. |
| **NWPS** | CSV (`.csv`) | NOAA OWP | National Water Prediction Service gauge metadata and flood stages[cite: 8]. |
| **GAGES-III** | CSV (`.csv`) | USGS | Environmental setting and screening metadata for USGS stream gages[cite: 8]. |
| **NWM Basins** | GeoPackage (`.gpkg`)| HydroShare | Basin boundary geometries for NWM assimilation streamgages[cite: 8]. |
...
1. Clone the repository:
   ```bash
   git clone [https://github.com/your-org/cyclone-streamflow.git](https://github.com/your-org/cyclone-streamflow.git)
...
```text
├── configuration.json        # Main configuration file containing URLs, paths, and metadata[cite: 8]
├── main.py                   # Primary pipeline execution entry point[cite: 12]
├── requirements.txt          # Package dependencies[cite: 15]
└── src/
    └── cyclone_streamflow/
        ├── __init__.py
        ├── configuration.py  # Pydantic schema models and column mappings[cite: 9]
        ├── data_processing.py# Geospatial transformation and cleanup routines[cite: 10]
        ├── data_retrieval.py # HTTP ingestion and USGS water data API clients[cite: 11]
        ├── manifest.py       # SQLite database manager for download checkpointing[cite: 13]
        └── pipelines.py      # Spatial buffer joins and sequence orchestrators[cite: 14]
